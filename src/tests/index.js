...
import { inspect, logger } from '../utils';
import Morphy from '../';
...
(async function () {
  ...
  await Promise.all(testsArguments.map(async ({ name, words, opts }, idx) => {
    name = name || idx;
...
//     let res = Object.keys(any).sort().reduce((res, key) => {
//       res[key] = consistentResults(any[key]);
//       // res[key] = ohash(consistentResults(any[key]));
//
//       return res;
//     }, {});
//
...
//   } else
//   if (_.isArray(any)) {
//     return any.map(consistentResults).sort();
//   }
//
//   return any;
//   // return ohash(any);
